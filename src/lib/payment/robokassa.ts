...
export function generateRobokassaLink(amount: number, invId: string, description: string, email: string) {
  ...
  const signature = crypto.createHash('md5').update(signatureValue).digest('hex');
...
export function validateRobokassaSignature(outSum: string, invId: string, signature: string) {
    if (!ROBOKASSA_PASS2) return false;
    
    const mySignatureValue = `${outSum}:${invId}:${ROBOKASSA_PASS2}`;
    const mySignature = crypto.createHash('md5').update(mySignatureValue).digest('hex');
