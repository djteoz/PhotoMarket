...
async function main() {
  ...
  try {
    ...
    console.log(
      `✅ Успешно! Пользователь ${user.email} теперь имеет права администратора (ADMIN).`
    );
    console.log(
      "Теперь вы можете зайти в панель управления: http://localhost:3000/admin"
    );
  } catch (e) {
    if (e.code === "P2025") {
      console.error(
        `❌ Ошибка: Пользователь с email "${email}" не найден в базе данных.`
      );
